...
describe('Players Controller', () => {
    ...
    describe('buildPlayersPage', () => {
            ...
            documentMock = {
                createElement: sinon.spy((elementType) => {
                    switch(elementType) {
                        ...
                        case 'tbody':
                            return tableBodyMock;
                        case 'tr':
                            return tableRowMock;
                        case 'th':
                            return tableHeaderElementMock;
                        case 'td':
                            return tableDataElementMock;
                    }
                }),
        ...
        it('should show the player stats for each season in a table in the modal', function () {
            ...
            assert.equal(documentMock.createElement.withArgs('tbody').callCount, 1);
            assert.equal(documentMock.createElement.withArgs('tr').callCount, 4);
            assert.equal(documentMock.createElement.withArgs('th').callCount, 4);
            assert.equal(documentMock.createElement.withArgs('td').callCount, 12);
            assert.equal(documentMock.createTextNode.callCount, 17);
            ...
            assert.equal(documentMock.createTextNode.args[5][0], 'season3');
            assert.equal(documentMock.createTextNode.args[6][0], 'some final fake value');
            assert.equal(documentMock.createTextNode.args[7][0], 9);
            assert.equal(documentMock.createTextNode.args[8][0], 5.24);
            assert.equal(documentMock.createTextNode.args[9][0], 'season2');
            assert.equal(documentMock.createTextNode.args[10][0], 'some other fake value');
            assert.equal(documentMock.createTextNode.args[11][0], 0);
            assert.equal(documentMock.createTextNode.args[12][0], 3.16);
            assert.equal(documentMock.createTextNode.args[13][0], 'season1');
            assert.equal(documentMock.createTextNode.args[14][0], 'some fake value');
            assert.equal(documentMock.createTextNode.args[15][0], 6);
            assert.equal(documentMock.createTextNode.args[16][0], 2.87);
            ...
            assert.equal(tableDataElementMock.appendChild.callCount, 12);
            assert.equal(tableDataElementMock.appendChild.withArgs(textNodeMock).callCount, 12);
            assert.equal(tableRowMock.appendChild.callCount, 16);
            assert.equal(tableRowMock.appendChild.withArgs(tableHeaderElementMock).callCount, 4);
            assert.equal(tableRowMock.appendChild.withArgs(tableDataElementMock).callCount, 12);
            ...
            assert.equal(tableBodyMock.appendChild.callCount, 3);
            assert.equal(tableBodyMock.appendChild.withArgs(tableRowMock).callCount, 3);
            assert.equal(tableMock.appendChild.callCount, 2);
            assert.equal(tableMock.appendChild.withArgs(tableHeaderMock).callCount, 1);
            assert.equal(tableMock.appendChild.withArgs(tableBodyMock).callCount, 1);
